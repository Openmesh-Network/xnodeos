  ...
  config = {
    ...
    system.build.kexecScript = lib.mkForce (
      pkgs.writeScript "kexec-boot" ''
        ...
        export OWNER="''${OWNER}" && export DOMAIN="''${DOMAIN}" && export EMAIL="''${EMAIL}" && export DEBUG="''${DEBUG}" && export VERSION_LOCK="''${VERSION_LOCK}" && export NETWORK="''${NETWORK}" && export INITIAL_CONFIG="''${INITIAL_CONFIG}"
        EOF
