  ...
  config = {
    ...
    system.build.kexecScript = lib.mkForce (
      pkgs.writeScript "kexec-boot" ''
        ...
        export OWNER="''${OWNER}" && export DOMAIN="''${DOMAIN}" && export EMAIL="''${EMAIL}" && export PASSWORD="''${PASSWORD}" && export ENCRYPTED="''${ENCRYPTED}" && export NETWORK="''${NETWORK}" && export INITIAL_CONFIG="''${INITIAL_CONFIG}"
        EOF
